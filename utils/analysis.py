...
def get_profile_analysis(df):
    ...
    try:
        def calculate_first_10_invalid_percentage(group):
            first_10 = group.head(10)
            if len(first_10) == 0:
                return 0.0
            invalid_count = first_10['Test_status'].isin(['Invalid', 'Indeterminate']).sum()
            return (invalid_count / len(first_10) * 100)
        ...
        # Calculate first 10 invalid percentage
        first_10_invalids = df.groupby(['Lab_name', 'Truelab_id']).apply(calculate_first_10_invalid_percentage)
        first_10_invalids = first_10_invalids.reset_index(name='First 10 Invalid %')
        ...
        detailed['First 10 Invalid %'] = detailed['First 10 Invalid %'].round(2)

        # Ensure all numeric columns are properly formatted
        numeric_columns = ['Total Tests', 'Total Detected', 'Total Invalid/Indeterminate',
                         'Invalid/Indeterminate %', 'First 10 Invalid %']
